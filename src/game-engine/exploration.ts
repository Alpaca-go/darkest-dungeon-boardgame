...
export function applyExplorationResult(
  ...
  switch (result) {
    ...
    case 'hunger':
      ...
      next = { ...next, heroes: next.heroes.map((h) => ({ ...h, wounds: h.wounds + 1 })) };
      return pushLog(next, '饥饿且补给耗尽，全队各受 1 Wound！', 'danger');
    ...
    case 'trap': {
      ...
      const victim = pick(next.heroes.filter((h) => h.isAlive));
      next = {
        ...next,
      ...
      return pushLog(next, '走廊陷阱且缺乏 Tool，随机英雄受 1 Wound，全队压力 +1！', 'danger');
    }
    ...
    case 'darkness':
      ...
      next = { ...next, heroes: next.heroes.map((h) => ({ ...h, stress: h.stress + 1 })) };
    ...
    case 'rubble': {
      ...
      return pushLog(next, '碎石且缺乏 Tool，随机英雄受 1 Wound，全队压力 +1！', 'danger');
    }
