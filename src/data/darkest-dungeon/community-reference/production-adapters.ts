...
import { COMMUNITY_ROOM9_EDGES, COMMUNITY_ROOM11_EDGES } from './community-source-geometry';
...
export const COMMUNITY_TEMPLARS_ROOM: TemplarsRoomDefinition = {
  ...
  // Phase 11A.4R1 WP-5：edges 由已接受 tileGeometry 轮廓派生（见 community-source-geometry）。
  areaGraph: { areas: templarAreas, edges: COMMUNITY_ROOM9_EDGES.map(([from, to]) => ({ from, to, distance: 1 })) },
...
export function validateCommunityTemplarsDefinitions(input: {
  ...
  // Phase 11A.4R1 WP-5：areaAdjacency blocker 已关闭（拓扑由已接受 tileGeometry 派生）；
  // 仅剩 Pit Exit 来源级 blocker（WP-6 SOURCE-BLOCKED）。
  return { isComplete: missing.length === 0 && issues.length === 0, missing, issues, knownBlockers: ['TEMPLARS_PIT_EXIT_RULE_UNRESOLVED'] };
}
