...
describe('Community Monster acceptance', () => {
  ...
  it('M03 production draw fills four physical instances from the shuffled pile', () => {
    ...
    const topFour = before!.drawPile.slice(0, 4);
    const result = drawDarkestDungeonMonster(campaign, () => 0);
    expect(result.ok).toBe(true);
    expect(result.campaign.actFourState.contentRuntime?.physicalMonsterDeck?.inBattle).toEqual(topFour);
    expect(result.campaign.actFourState.contentRuntime?.physicalMonsterDeck?.activePlacements).toHaveLength(4);
    expect(result.monsterDefinitionId).toBe(before!.instanceToDefinitionId[topFour[0]]);
  });
  ...
  it('M05 mutates only the physical draw pile identity for the four-slot fill', () => {
    const campaign = createCommunityGuardianScenario(0);
    const topFour = campaign.actFourState.contentRuntime!.physicalMonsterDeck!.drawPile.slice(0, 4);
    expect(drawDarkestDungeonMonster(campaign, () => 0).campaign.actFourState.contentRuntime!.physicalMonsterDeck!.inBattle).toEqual(topFour);
  });
