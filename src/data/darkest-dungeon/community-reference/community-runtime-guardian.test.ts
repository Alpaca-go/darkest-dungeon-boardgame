...
describe('Community Guardian semantic acceptance', () => {
  ...
  it('G12 spawn and no-space semantics are classified', () => expect(COMMUNITY_RUNTIME_FIELD_COVERAGE.find((entry) => entry.requirementId === 'tierB-mammoth-cyst-room' && entry.field === 'spawnAreaPolicy')?.runtimeClassification).toBe('consumed'));
