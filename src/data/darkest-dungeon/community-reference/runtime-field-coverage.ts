...
for (const req of NORMALIZED_CORPUS.requirements) {
  for (const field of Object.keys(req.fields)) {
    ...
    if (req.requirementId.startsWith('tierB-quest-')) {
      ...
      if (field === 'provisionPolicyId.cardSpecific') { add(req.requirementId, field, 'consumed', { field, runtimeSelectorId: 'quest.provisionPolicy', runtimeSelector: () => COMMUNITY_QUEST_PROVISION_POLICY, normalizeSource: (value: any) => ({ cardSpecificOverride: value.cardSpecificOverride, dicePerHero: 2, wildChoice: true, commonPoolMaximum: 16 }), normalizerId: 'community-provision-policy.v1' }); continue; }
    ...
    if (req.requirementId === 'tierB-mammoth-cyst-room') {
      if (field === 'spawnAreaPolicy') { for (const part of ['mammothInitialArea','summonPlacement','stanceToArea','noSpace']) { const path = `${field}.${part}`; if (part === 'noSpace') add(req.requirementId, path, 'consumed', { field, sourceSelector: environment => sourceSubValue(req.requirementId, field, part, environment), runtimeSelectorId: `mammothRoom.${path}`, runtimeSelector: env => ({ policy: env.mammothRoom.areaGraph.edges.length > 0 ? 'nearest-available-displacement' : 'missing-topology', heroFirst: true, monsterPlayerChoice: true }), normalizeSource: (value) => { const text = String(value); return /Hero must move to nearest available Area/i.test(text) && /players choose a Monster/i.test(text) ? { policy: 'nearest-available-displacement', heroFirst: true, monsterPlayerChoice: true } : { policy: 'unrecognized', raw: text }; }, normalizerId: 'room11-no-space-displacement.v1' }); else add(req.requirementId, path, 'consumed', { field, sourceSelector: environment => sourceSubValue(req.requirementId, field, part, environment), runtimeSelectorId: `mammothRoom.${path}`, runtimeSelector: env => part === 'mammothInitialArea' ? env.mammothRoom.mammothCystPlacement.areaId : part === 'stanceToArea' ? env.mammothRoom.stanceAreaMap : env.mammothRoom.whiteCellStalkSpawn.areaPolicy, normalizeSource: part === 'summonPlacement' ? normalizeSummonPlacement : identity, normalizerId: part === 'summonPlacement' ? 'spawn-area-policy.v2' : 'identity.v1' }); } }
