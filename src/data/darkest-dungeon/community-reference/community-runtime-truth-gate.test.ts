...
describe('Community Runtime Acceptance Truth Gate adversarial mutations', () => {
  ...
  it('T05 unproven Final skill mapping cannot be treated as consumed', () => { const proof = COMMUNITY_RUNTIME_PROJECTION_PROOFS.find(item => item.requirementId === 'tierB-ancestor-first-form' && item.sourcePath === 'vacantStanceFillSource')!; expect(proof.classification).toBe('engine-unsupported-blocker'); expect(proof.blockerCode).toBe('FINAL_SKILL_TABLE_ENGINE_UNSUPPORTED'); });
  ...
  it('T09 compound parent consumed above blocked child fails', () => { const child = COMMUNITY_RUNTIME_PROJECTION_PROOFS.find(proof => proof.sourcePath === 'spawnAreaPolicy.noSpace')!; const parent = { ...child, sourcePath: 'spawnAreaPolicy', classification: 'consumed' as const, runtimeSelectorId: 'forged.parent', runtimeSelector: () => child.sourceSelector() }; expect(validateCommunityRuntimeProjectionProofs([...COMMUNITY_RUNTIME_PROJECTION_PROOFS, parent]).some(error => error.includes('compound parent consumed'))).toBe(true); });
...
describe('Mammoth source-confirmed effects and deterministic closure', () => {
  ...
  it('T14 full spawn Area reaches the precise blocker without RNG or mutation', () => {
    const campaign = createCommunityGuardianScenario(2); const state = campaign.actFourState.mammothCystEncounterState!; const card = state.initiativeCards.find(item => item.owner === 'mammoth-cyst')!; const area = state.snapshot.room.stanceAreaMap.ranged!; const filled = { ...state, heroPlacements: Array.from({ length: state.snapshot.room.areaCapacities[area] }, (_, index) => ({ heroId: `fill-${index}`, areaId: area })) }; const checkpoint = { ...campaign, actFourState: { ...campaign.actFourState, mammothCystEncounterState: filled } }; const before = jsonRoundTrip(checkpoint); const result = summonWhiteCellStalk(checkpoint, { mode: 'community-reference', sourceActionEventId: card.id, rng: () => { throw new Error('RNG consumed'); } }); expect(result.blockerCode).toBe('MAMMOTH_STALK_NO_SPACE_RESOLUTION_ENGINE_UNSUPPORTED'); expect(result.campaign).toEqual(before);
  });
  it('T15 full-area blocker survives save/reload identically', () => {
    const campaign = createCommunityGuardianScenario(2); const state = campaign.actFourState.mammothCystEncounterState!; const card = state.initiativeCards.find(item => item.owner === 'mammoth-cyst')!; const area = state.snapshot.room.stanceAreaMap.ranged!; const filled = { ...state, heroPlacements: Array.from({ length: state.snapshot.room.areaCapacities[area] }, (_, index) => ({ heroId: `fill-${index}`, areaId: area })) }; const checkpoint = { ...campaign, actFourState: { ...campaign.actFourState, mammothCystEncounterState: filled } }; const options = { mode: 'community-reference' as const, sourceActionEventId: card.id, rng: () => 0 }; expect(summonWhiteCellStalk(jsonRoundTrip(checkpoint), options).blockerCode).toBe(summonWhiteCellStalk(checkpoint, options).blockerCode);
  });
});
