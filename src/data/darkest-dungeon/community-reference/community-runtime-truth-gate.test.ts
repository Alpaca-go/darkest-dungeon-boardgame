...
describe('Community Runtime Acceptance Truth Gate adversarial mutations', () => {
  ...
  it('T05 Final vacant-stance fill is consumed by the Community d10 table', () => { const proof = COMMUNITY_RUNTIME_PROJECTION_PROOFS.find(item => item.requirementId === 'tierB-ancestor-first-form' && item.sourcePath === 'vacantStanceFillSource')!; expect(proof.classification).toBe('consumed'); expect(proof.blockerCode).toBeUndefined(); });
