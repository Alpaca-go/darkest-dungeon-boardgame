...
describe('Community Final Encounter acceptance', () => {
  ...
  it('F03 Final Provision blocks before mutation', () => { const campaign = finalReady(); const before = structuredClone(campaign); expect(prepareFinalEncounter(campaign, { mode: 'community-reference', rng: () => 0 })).toMatchObject({ campaign: before, ok: false }); });
  ...
  it('F09 Ancestor second Nothingness occupants are non-acting and teleport can run', () => {
    const campaign = withForm('ancestor-second-form', 'f09');
    const runtime = campaign.actFourState.finalFormRuntimeState?.runtimes['ancestor-second-form'];
    expect(runtime).toMatchObject({ kind: 'ancestor-second-form' });
    if (runtime?.kind === 'ancestor-second-form') {
      expect(runtime.nothingness).toHaveLength(3);
      expect(runtime.nothingness.every((item) => item.stanceTrackerStance === null && item.targetable === false && item.occupiesAreaSpace === true)).toBe(true);
    }
