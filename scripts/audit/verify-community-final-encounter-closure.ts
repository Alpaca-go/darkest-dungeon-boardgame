/**
 * Phase 11A.4R2 — Community Final Encounter production closure measured verification.
 * Writes LOCAL MEASURED VERIFICATION evidence under docs/reports/phase-11a4r2/.
...
const OUTPUT = 'docs/reports/phase-11a4r2';
const EVIDENCE = 'docs/data/darkest-dungeon/community-reference/community-final-encounter-closure-evidence.json';
const REPORT = `${OUTPUT}/phase-11a4r2-community-final-encounter-closure-report.md`;
...
if (dirty) {
  throw new Error('Working tree must be clean before measured R2 verification');
}
...
run('finalBrowserE2E', ['scripts/e2e/run-community-act4-playable-closure-e2e.mjs'], 300_000, {
...
try {
  ...
  failures.push(`finalBrowserE2E: unreadable report (${String(error)})`);
}
const e2e = {
  expected: 3,
...
if (e2e.discovered !== e2e.expected || e2e.passed !== e2e.expected || e2e.failed) {
  failures.push(`finalBrowserE2E: structured count mismatch ${JSON.stringify(e2e)}`);
}
...
const evidence = {
  schemaVersion: 'phase11a4r2-community-final-encounter-closure.v1',
  runId: randomUUID(),
  ...
  failures,
  acceptanceNotes: [
    'R2 implementation acceptance complete when terminalVerdict is COMMUNITY-FINAL-RUNTIME-IMPLEMENTATION-ACCEPTED.',
    ...
    'Do not start Phase 11B or R3 Full Route Acceptance until remaining source blockers are resolved from accepted sources or product policy.',
  ],
};
...
writeFileSync(
  resolve(ROOT, REPORT),
  `# Phase 11A.4R2 Community Final Encounter Production Closure
...
| finalBrowserE2E | ${e2e.expected} | ${e2e.passed} | ${e2e.failed} |
