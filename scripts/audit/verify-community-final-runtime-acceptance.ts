/**
 * Phase 11A.4R2A — Community Final runtime acceptance repair measured verification.
 * Writes LOCAL MEASURED VERIFICATION evidence under docs/reports/phase-11a4r2a/.
 * Does not claim CI green. Requires a clean tracked working tree at the implementation HEAD.
 */
...
const OUTPUT = 'docs/reports/phase-11a4r2a';
const EVIDENCE = 'docs/data/darkest-dungeon/community-reference/community-final-runtime-acceptance-evidence.json';
const REPORT = `${OUTPUT}/phase-11a4r2a-community-final-runtime-acceptance-report.md`;
...
const SUITES = {
  sourceEvidenceBinding: {
    file: 'src/data/darkest-dungeon/community-reference/community-final-skill-source-evidence.test.ts',
    expected: 9,
  },
  finalSkillCoverage: {
    file: 'src/data/darkest-dungeon/community-reference/community-final-skill-coverage.test.ts',
    expected: 5,
  },
  finalProduction: {
    file: 'src/data/darkest-dungeon/community-reference/community-final-production.test.ts',
    expected: 25,
  },
  finalTransitionProduction: {
    file: 'src/data/darkest-dungeon/community-reference/community-final-transition-state.test.ts',
    expected: 2,
  },
  finalSaveReplay: {
    file: 'src/data/darkest-dungeon/community-reference/community-final-encounter-save-replay.test.ts',
    expected: 18,
  },
  finalAdversarial: {
    file: 'src/data/darkest-dungeon/community-reference/community-final-encounter-adversarial.test.ts',
    expected: 41,
  },
  segmentRouteMatrix: {
  ...
  frozenFinal: {
    file: 'src/data/darkest-dungeon/community-reference/community-runtime-final.test.ts',
    expected: 12,
  },
} as const;
...
if (dirty) {
  throw new Error('Working tree must be clean before measured R2A verification');
}
...
run('browserE2E', ['scripts/e2e/run-community-act4-playable-closure-e2e.mjs'], 300_000, {
...
try {
  ...
  failures.push(`browserE2E: unreadable report (${String(error)})`);
}
const e2e = {
  expected: 3,
...
if (e2e.discovered !== e2e.expected || e2e.passed !== e2e.expected || e2e.failed) {
  failures.push(`browserE2E: structured count mismatch ${JSON.stringify(e2e)}`);
}
...
const terminalVerdict = failures.length === 0
  ? 'COMMUNITY-FINAL-RUNTIME-IMPLEMENTATION-ACCEPTED'
  : 'FAILED';

const evidence = {
  schemaVersion: 'phase11a4r2a-community-final-runtime-acceptance.v1',
  runId: randomUUID(),
  ...
  terminalVerdict,
  sourceBlocked: [...EXPECTED_BLOCKERS],
  ...
  failures,
  acceptanceNotes: [
    'R2A repaired the R2 false-green: FINAL_SKILL_TABLE_ENGINE_UNSUPPORTED and FINAL_ROOM_TRANSITION_ENGINE_UNSUPPORTED were re-opened (WP-0) and re-closed only after independent source evidence binding, hardened coverage contract (no metadata-only proofs), source-backed range/target semantics, shared mark/stress pipelines, cross-form hero combat state preservation, transition + leaf-level mutation gates, and the extended save/replay matrix all passed.',
    'R2A acceptance complete when terminalVerdict is COMMUNITY-FINAL-RUNTIME-IMPLEMENTATION-ACCEPTED.',
    'Remaining SOURCE-BLOCKED rules: TEMPLARS_PIT_EXIT_RULE_UNRESOLVED, GESTATING_HEART_LETHAL_TIMING_UNRESOLVED, COME_UNTO_YOUR_MAKER_UNRESOLVED.',
    'Route matrix evidence is Guardian->Final segment integration, NOT full Quest->Campaign route acceptance; full route acceptance is reserved for R3.',
    'This evidence is LOCAL MEASURED VERIFICATION; it does not claim CI green.',
    'Do not start Phase 11B or R3 Full Route Acceptance until remaining source blockers are resolved from accepted sources or product policy. STOP for independent audit.',
  ],
};
...
writeFileSync(
  resolve(ROOT, REPORT),
  `# Phase 11A.4R2A Community Final Runtime Acceptance Repair
...
- Evidence publication parent: \`${verifiedImplementationHead}\`
- SOURCE-BLOCKED: ${EXPECTED_BLOCKERS.map((code) => `\`${code}\``).join(', ')}
...
| browserE2E | ${e2e.expected} | ${e2e.passed} | ${e2e.failed} |
...
if (failures.length) {
  console.error(JSON.stringify({ terminalVerdict, failures, verifiedImplementationHead }, null, 2));
  process.exit(1);
}

console.log(JSON.stringify({ terminalVerdict, verifiedImplementationHead, evidence: EVIDENCE, report: REPORT }, null, 2));
